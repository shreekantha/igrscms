...
export class HomeImgComponent implements OnInit, OnDestroy {
  ...
  loadPage(page?: number, dontNavigate?: boolean): void {
    ...
    this.homeImgService
      ...
      .subscribe(
        (res: HttpResponse<IHomeImg[]>) => this.onSuccess(res.body, res.headers, pageToLoad, !dontNavigate),
        () => this.onError()
      );
  }
  ...
  registerChangeInHomeImgs(): void {
    this.eventSubscriber = this.eventManager.subscribe('homeImgListModification', () => this.loadPage());
  }
  ...
  protected onSuccess(data: IHomeImg[] | null, headers: HttpHeaders, page: number, navigate: boolean): void {
    this.totalItems = Number(headers.get('X-Total-Count'));
    this.page = page;
    if (navigate) {
      this.router.navigate(['/home-img'], {
        queryParams: {
          page: this.page,
          size: this.itemsPerPage,

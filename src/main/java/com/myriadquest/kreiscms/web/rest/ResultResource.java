 ...
 * {@link com.myriadquest.kreiscms.domain.SslcResult}.
 */
@RestController
@RequestMapping("/api")
public class ResultResource {
	...
	public ResultResource(SslcResultService sslcResultService, SslcResultQueryService sslcResultQueryService) {
	 ...
	 *         body the new sslcResultDTO, or with status {@code 400 (Bad Request)}
	 *         if the sslcResult has already an ID.
	 * @throws URISyntaxException if the Location URI syntax is incorrect.
	 */
	@PostMapping("/results")
	public ResponseEntity<SslcResultDTO> createSslcResult(@Valid @RequestBody SslcResultDTO sslcResultDTO)
		...
		return ResponseEntity
				.created(new URI("/api/sslc-results/" + result.getId())).headers(HeaderUtil
	 ...
	 *         the updated sslcResultDTO, or with status {@code 400 (Bad Request)}
	 *         if the sslcResultDTO is not valid, or with status
	 *         {@code 500 (Internal Server Error)} if the sslcResultDTO couldn't be
	 *         updated.
	 * @throws URISyntaxException if the Location URI syntax is incorrect.
	 */
	@PutMapping("/results")
	public ResponseEntity<SslcResultDTO> updateSslcResult(@Valid @RequestBody SslcResultDTO sslcResultDTO)
			throws URISyntaxException {
		...
		return ResponseEntity.ok().headers(HeaderUtil.createEntityUpdateAlert(applicationName, true, ENTITY_NAME,
				sslcResultDTO.getId().toString())).body(result);
	}
	 ...
	 *         of sslcResults in body.
	 */
	@GetMapping("/results")
	public ResponseEntity<Map<String, Object>> getAllResults() {
		SslcResultCriteria criteria = new SslcResultCriteria();
		PucResultCriteria pucResultCriteria = new PucResultCriteria();
		log.debug("REST request to get SslcResults by criteria: {}", criteria);
		criteria.setTenantId(IgrscmsApp.getTenantFilter());
		pucResultCriteria.setTenantId(IgrscmsApp.getTenantFilter());

		List<SslcResultDTO> entityList = sslcResultQueryService.findByCriteria(criteria);
		List<PucResultDTO> pucEntityList = pucResultQueryService.findByCriteria(pucResultCriteria);

		Map<String, Object> result = new HashMap<String, Object>();
		result.put("sslc", entityList);
		result.put("puc", pucEntityList);
	 ...
	 * @return the {@link ResponseEntity} with status {@code 200 (OK)} and with body
	 *         the sslcResultDTO, or with status {@code 404 (Not Found)}.
	 */

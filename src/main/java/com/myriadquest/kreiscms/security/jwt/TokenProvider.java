package com.myriadquest.kreiscms.security.jwt;
...
public class TokenProvider {
	...
	public String createToken(com.myriadquest.kreiscms.domain.User authentication, boolean rememberMe) {
		log.error("user++++++++++++++++++++++++++++++++++++:{}",authentication.getLogin());
		String authorities = authentication.getAuthorities().stream().map(Authority::getName)
				.collect(Collectors.joining(","));
		...
		return Jwts.builder().setSubject(authentication.getLogin()).claim(AUTHORITIES_KEY, authorities)
				.claim("tenantId", authentication.getTenantId()).signWith(key, SignatureAlgorithm.HS512)
				.setExpiration(validity).compact();
	}

	public Authentication getAuthentication(String token) {
		Claims claims = Jwts.parserBuilder().setSigningKey(key).build().parseClaimsJws(token).getBody();
log.info("claims:==================================:{}:{}",claims,token); 
